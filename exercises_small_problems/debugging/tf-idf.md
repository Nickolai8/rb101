...
The issue with the above code is that on `line 26`, the code is performing integer division between local variables `number_of_documents` and `number_of_documents_with_term`. This should be float division, as it is dramatically influencing the outcome of taking the log base 10 of a number. In many cases, the integer division is returning 1, which log base anything 1 always returns 0.  Doing float division will correct this.
...
def idf(term, documents)
  number_of_documents = documents.length * 1.0
  number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 }
